...
One deliberate departure from textbook PID: once the controller has decided which direction to move, it adds a fixed minimum-speed offset before sending the command to the motor driver. The gear motor has enough static friction that small commanded speeds near zero just stall rather than moving — a plain PID output tends to hover in that dead zone as it approaches the goal. The minimum-speed kick keeps the motor actually turning through the whole approach, which is what gets the steady-state accuracy down.
...
- [`code-esp32/`](code-esp32) — PlatformIO firmware project targeting a Freenove ESP32-S3, Arduino framework.
  ...
  - [`src/controller.*`](code-esp32/src/controller.cpp) — the control box: buttons, display, and the AS5600 rotary knob.
